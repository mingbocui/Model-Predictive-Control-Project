...
dim   = 100; % Number of optimization variables
...
%     % A very shord solution for computation of search direction:
%     %vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv
%     % Compute search direction 
%     s = prob.d - prob.G*z;
%     Dz = -inv(prob.H + kappa*prob.G'*diag(1./(s.*s))*prob.G)*...
%       (prob.H*z+prob.q + kappa*prob.G'*diag(1./s)*ones(length(s),1));
%     %^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

    % Longer, but easier to read solution:
    %vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv
    % Compute the sum on the left side
    leftSum = 0;
    for k = 1 : length( prob.d )
        leftSum = leftSum + ( 1 / ( prob.d(k) - prob.G(k,:) * z )^2 ) * prob.G(k,:)' * prob.G(k,:); 
    end
    % Compute the matrix on the left side
    leftTerm = prob.H + kappa * leftSum;
    
    % Compute the sum on the right side
    rightSum = 0;
    for k = 1 : length( prob.d )
        rightSum = rightSum + ( 1 / ( prob.d(k) - prob.G(k,:) * z ) ) * prob.G(k,:)';
    end
    % Compute the vector on the right side
    rightTerm = - prob.H * z - prob.q - kappa * rightSum;
    
    % Compute the direction
    Dz = leftTerm\rightTerm;
    %^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
% TODO what is dim, why multiply 5?
prob.dim = dim;
